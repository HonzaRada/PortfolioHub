...
export function DashboardView({ 
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <nav className="sticky top-0 z-30 border-b border-slate-200 bg-white/80 backdrop-blur-md px-6 py-3">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          ...
          <div className="flex items-center gap-4">
             ...
             <button
              onClick={() => signOut()}
              className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-red-600 transition"
            >
              Odhlásit
            </button>
          </div>
      ...
      <main className="mx-auto max-w-7xl p-6 lg:p-8">
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 className="text-2xl font-bold text-slate-900">Přehled transakcí</h1>
                <p className="text-slate-500">Zde vidíš historii všech svých investic.</p>
            </div>
            
            {/* Tlačítko pro novou transakci (později ho oživíme) */}
            <button className="rounded-xl bg-indigo-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700 transition">
                + Nová transakce
            </button>
        </div>
                  ...
                  <tr>
                    <td colSpan={6} className="px-6 py-16 text-center">
                      <div className="flex flex-col items-center justify-center gap-2">
                        ...
                        <p className="font-medium text-slate-900">Zatím žádná data</p>
                        <p className="text-slate-500">Přidej svou první transakci tlačítkem nahoře.</p>
                      </div>
                    </td>
                  </tr>
                ) : (
