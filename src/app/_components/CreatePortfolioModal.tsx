...
type Props = {
    isOpen: boolean;
    onClose: () => void;
    initialData?: PortfolioData | null; // Přijímá data pro editaci
};
...
export function CreatePortfolioModal({ isOpen, onClose, initialData }: Props) {
    ...
    // Efekt pro předvyplnění formuláře, když upravujeme existující portfolio
    useEffect(() => {
        if (isOpen) {
            if (initialData) {
                // Použijeme reset místo setValue - to smaže staré chybové hlášky!
                reset({ name: initialData.name });
            } else {
                reset({ name: "" });
            }
        } else {
            // Když se modál zavře (tlačítko Zrušit), formulář rovnou vyčistíme
            reset({ name: "" });
        }
